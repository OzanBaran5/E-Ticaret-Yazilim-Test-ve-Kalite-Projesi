...
public class IntegrationTests
{
    ...
    /// <summary>
    /// PASS: Başarılı sipariş sonrası cart.Status "Checkout", stok düşmüş ve order başarılı.
    /// Tüm modüllerin (Product + Cart + OrderService) birlikte doğru çalıştığını doğrular.
    /// EP: Geçerli tam akış senaryosu — stoklu ürün, yeterli ödeme.
    /// </summary>
    [Test]
    [Category("Integration Test")]
    public void FullFlow_AfterSuccessfulOrder_StatusAndStockCorrect()
    {
        var product = new Product { Id = 105, Name = "Kulaklık", Price = 200m, Stock = 5 };
        var cart = new Cart();
        var service = new OrderService();
        cart.AddProduct(product);
